...
```{r plots3}
fst3 <- read.table('FST/P-L-SUO.windowed.weir.fst',
                  header = TRUE, stringsAsFactors = FALSE)
...
```{r plots4}
fst4 <- read.table('FST/P-L-LAN.windowed.weir.fst',
                  header = TRUE, stringsAsFactors = FALSE)
...
```{r comparacio2}
# SUO:
thrs3 <- quantile(fst3$WEIGHTED_FST, probs = 0.995)
f3 <- fst3$WEIGHTED_FST >= thrs3
# LAN:
thrs4 <- quantile(fst4$WEIGHTED_FST, probs = 0.995)
...
GRn <- data.frame(
   ...
   lake = c(rep('Thun-Brienz', length(GR3)),
            rep('Walen-Zurich', length(GR4))),
