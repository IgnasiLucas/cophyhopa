...
author: "J. Ignacio Lucas Lledó & Mar Llaberia-Robledillo"
date: "`r Sys.Date()`"
output:
   html_document:
      toc: TRUE
...
ggplot(data = fst1, mapping = aes(x = POS2, y = WEIGHTED_FST, color = ChromCol)) +
   ...
   xlab('Position') + ylab(expression("Weighted "*F[ST])) +
   ggtitle('Thun and Brienz lakes, C. fatioi and C. steinmanni') +
...
ggplot(data = fst2, mapping = aes(x = POS2, y = WEIGHTED_FST, color = ChromCol)) +
   geom_point() + theme(legend.position="none") +
   xlab('Position') + ylab(expression("Weighted "*F[ST])) +
   ggtitle('Walen and Zurich lakes, C. zuerichensis and C. duplex') +
   theme(axis.text.x = element_blank(), axis.ticks.x = element_blank())
```
...
above that threshold in both lakes. The threshold is the 99$^\text{th}$ percentile.
...
ggplot(data = fst3, mapping = aes(x = POS2, y = WEIGHTED_FST, color = ChromCol)) +
   geom_point() + theme(legend.position="none") +
   xlab('Position') + ylab(expression("Weighted "*F[ST])) +
   ggtitle('Suopatjavri, P and L ecomorphs') +
   theme(axis.text.x = element_blank(), axis.ticks.x = element_blank())
```
...
ggplot(data = fst4, mapping = aes(x = POS2, y = WEIGHTED_FST, color = ChromCol)) +
   geom_point() + theme(legend.position="none") +
   xlab('Position') + ylab(expression("Weighted "*F[ST])) +
   ggtitle('Langfjordvatn, P and L ecomorphs') +
   theme(axis.text.x = element_blank(), axis.ticks.x = element_blank())
```
