...
files, to remove poorly mapped readsk. And I need to decide on the ancestral
...
for sp in $(cat species.txt); do
   ...
   for sample in $(cat $sp/samples.txt); do
      if [ ! -e $sp/$sample.bam ]; then
         ...
         samtools view -u - | \
         samtools addreplacerg -o $sp/$sample.bam -r "@RG\tID:$sample\tSM:$sample" - &
      fi
   done
   wait
   for sample in $(cat $sp/samples.txt); do
...
Note: freebayes does not work. I may use samtools mpileup and bcftools call
(although "calling" would bias the AFS...)
...
for sp in $(cat species.txt); do
   if [ ! -e $sp/$sp.vcf ]; then
      ...
      # -g 1200: maximum coverage. For all species, 1200 is above or about
      # three standard deviations above the average coverage of the
      # sample with highest coverage.
