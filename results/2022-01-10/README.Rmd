...
I tried use 40 threads in clustering, but I run out of RAM. If using only one
thread does not solve the memory problem, I will have to separate merged from non
merged loci. Apparently, VSEARCH is not very efficient with datasets other than
amplicons. The excess of diversity in random loci consumes too much memory.

```{bash clustering, eval=FALSE}
if [ ! -e centroids.fa ]; then
   if [ ! -e sorted.fa ]; then
      cat derep/*.fa | \
   ...
   vsearch --cluster_smallmem sorted.fa \
...
Unfortunately, the clustering step consumes too much memory, even
after reducing the number of threads to 1, skipping reads shorter
than 100 nucleotides, and requesting only the centroids, instead of
the consensus and the tabulated output.
