...
setwd("~/cophyhopa/results/2022-05-09/fineRADstructure/") ## The directory where the files are located
chunkfile<-"assem2_filter.recode.p.haps_chunks.out" ## RADpainter output file
...
plotsFolder <- "~/cophyhopa/results/2022-05-09/fineRADstructure/plots/"
### 3) SET VALUES FOR THESE VARIABLES: "analysisName" will be included in output plots
analysisName <- "populations";  maxIndv <- 10000 ; maxPop <- 10000


### 4) EDIT THE PATH TO YOUR COPY of FinestructureLibrary.R
source("~/cophyhopa/results/2022-05-09/fineRADstructure/FinestructureLibrary.R", chdir = TRUE) # read in the R functions, which also calls the needed packages
...
dataraw <- as.matrix(read_delim("assem2_filter.recode.p.haps_chunks.out", delim = "\t", escape_double = FALSE, trim_ws = TRUE, skip = 1))
row.names(dataraw)<- dataraw[, 1]
dataraw <- dataraw[, -1]
