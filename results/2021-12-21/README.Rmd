...
for file in $(ls -1 $DATADIR/fastq/*.fastq.gz); do
   ...
   if ! $(echo $SAMPLE | grep -q HOP); then
      if [ ! -e run1/${SAMPLE}_${READ}_.fastq.gz ]; then
         ln -s $file run1/${SAMPLE}_${READ}_.fastq.gz
      fi
   fi
done
...
for file in $(ls -1 $DATADIR/fastr/*.fastq.gz); do
   ...
   READ=$(echo $file | cut -d "_" -f 4)
   if ! $(echo $SAMPLE | grep -q HOP); then
      if [ ! -e run2/${SAMPLE}_${READ}_.fastq.gz ]; then
         ln -s $file run2/${SAMPLE}_${READ}_.fastq.gz
      fi
   fi
done
```
