...
output: 
   html_document:
      toc: true
---
# All regions admixture analysis (K = 2 3 4 5 6 7 8)
```{bash, results = "hide"}
...
```{bash, results = "hide"}
INPUT='/gata/mar/cophyhopa/results/2023-09-13/ADMIX/AREG.bed'
...
```{bash echo = TRUE}
grep -h CV log*.AREG.out
```

# Alpine admixture analysis (K = 6, 7, 8)
```{bash, results = "hide"}
VCF='/gata/mar/cophyhopa/results/2022-04-22/assem2_outfiles/assem2.vcf'
...
```{bash, results = "hide"}
INPUT='/gata/mar/cophyhopa/results/2023-09-13/ADMIX/ALPINE.bed'
...
```{bash echo = TRUE}
grep -h CV log*.ALPINE.out
```

# Arctic admixture analysis (K = 2, 3)
